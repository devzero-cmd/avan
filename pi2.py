...
OBSTACLE_CLASSES = {
    "door", "stairs", "wall", "person", "vehicle", "fence", "pole",
    "chair", "sofa", "bench", "couch", "table", "bed", "bicycle",
    "motorbike", "trash can", "bag", "backpack"
}
...
# Use Pi Camera (CSI) instead of webcam
cap = cv2.VideoCapture(0)
if not cap.isOpened():
    # Try alternative camera index or GStreamer pipeline for Pi Camera
    cap = cv2.VideoCapture(1)
    
if not cap.isOpened():
    # Use GStreamer pipeline for Raspberry Pi CSI camera
    gstreamer_pipeline = (
        "nvarguscamerasrc ! "
...
if not cap.isOpened():
    raise RuntimeError("Could not open camera (CSI or webcam).")
...
def draw_guidance(frame, msg, safe_dir):
    ...
    cv2.putText(frame, msg, (10, h - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cx, cy = w // 2, h // 2
    if safe_dir == "left":
        pts = np.array([[cx - 60, cy], [cx + 10, cy - 40], [cx + 10, cy + 40]], np.int32)
        cv2.fillPoly(frame, [pts], (0, 255, 255))
    elif safe_dir == "right":
        pts = np.array([[cx + 60, cy], [cx - 10, cy - 40], [cx - 10, cy + 40]], np.int32)
        cv2.fillPoly(frame, [pts], (0, 255, 255))
    else:
        pts = np.array([[cx, cy - 60], [cx - 40, cy + 10], [cx + 40, cy + 10]], np.int32)
        cv2.fillPoly(frame, [pts], (0, 255, 255))
...
try:
    while True:
        ...
        # Flip camera if needed (remove if camera is already oriented correctly)
        ...
        hist_array = np.array([[1 if s == "blocked" else 0 for s in f] for f in history])
        smoothed = np.mean(hist_array, axis=0) if hist_array.size else np.array([0, 0, 0])
        status = ["blocked" if s > 0.5 else "clear" for s in smoothed]

        msg = "Path clear"
        safe_dir = "forward"

        if wall_detected or (status[0] == "blocked" and status[1] == "blocked" and status[2] == "blocked"):
            ...
            if status[1] == "blocked":
                if status[0] == "clear":
                    ...
                    block_count += 1
                    msg = "Stop, obstacle unavoidable" if block_count > 3 else "Keep forward"
        ...
        if DEBUG:
            ...
            for x1, y1, x2, y2, label, conf_val, is_blocker in debug_boxes:
                x1i, y1i, x2i, y2i = int(x1), int(y1) + h // 4, int(x2), int(y2) + h // 4
                color = (0, 0, 255) if is_blocker else (255, 165, 0)
                cv2.rectangle(frame, (x1i, y1i), (x2i, y2i), color, 2)
                cv2.putText(frame, f"{label} {conf_val:.2f}", (x1i, y1i - 6),
